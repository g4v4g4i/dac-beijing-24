...
\begin{frame}{}
    ...
    \begin{tikzpicture}[scale=.7, every node/.style={ospale, scale=.8}, node distance=.6cm and .3cm]
        ...
        \node [os, right = of b1] (b2) {\afnodeflat{b_2}{\Monday^f,\burger^h,\d_6}{\neg\d_2}};
...
\draw[darr] (mc.0) to [bend left] node[above] {\( \delta_4 \)} (cash);
\draw[darr] (luigi.0) to [bend right] node[above] {\( \delta_5 \)} (cash);
\draw[green,darr] (Monday.0) to [bend left] node[above,pos=.4] {\( \delta_6 \)} node [sloped] {\( \mid \)} (mc);
\end{tikzpicture}  
\end{frame}
...
\begin{frame}{}
    Compare to DAC with OR we may loose consequences, such as $\mathsf{cash}^b$ in this case.
...
\draw[green,darr] (Monday.0) to [bend left] node[above,pos=.4] {\( \delta_6 \)} (cash);
\draw[green,darr] (Monday) to node[above] {$\delta_5$}node[sloped] {$\mid$}  (mc);
\end{tikzpicture}  
\end{frame}
...
\begin{frame}{Hypothetical consequences}
    ...
    \begin{itemize}
        \item<2-> $\alert<2-3>{\sf pizza} \leadsto {\sf Luigi}$
        \item<3-> $\alert<3>{\sf pizza} \leadsto {\sf cash}$
        \item<4-> $\alert<4-5>{\sf burger} \leadsto {\sf KingBurger}$
        \item<5-> $\alert<5>{\sf burger} \leadsto \neg{\sf cash}$ 
    \end{itemize}
    \end{column}
...
\scalebox{1}{
    \begin{tikzpicture}[scale=.8, every node/.style={scale=.8, align=center}, rounded corners, decorate, node distance = .3cm and 1.8cm]
\node [] (v) {\( \vee \)};
\node [ndnode, above = of v] (burger) {\alert<4-5>{\sf burger}};
\node [ndnode, below = of v] (pizza) {\alert<2-3>{\sf pizza}};
\begin{scope}[on background layer]
  \node [dnode, fit={(burger) (v) (pizza)}] (vee) {};
\end{scope}
\node [dnode, left = of vee] (lunch) {\sf Lunch};
...
\node [above = 1.3cm of vee, dnode] (nMc) {$\neg$ {\sf McBurger}};
\draw[-] (burger) to node[ospale] (wedge) {$\wedge$} (nMc);
\node [ospale,right = of wedge] (king) {\alert<4-5>{\sf KingBurger}};
\draw[darr] (wedge) to node[above] {$\delta_2$} (king);
\node [ospale,right = of king] (ncash) {\alert<3>{\alert<5>{$\neg$ {\sf cash}}}};
\node [ndnode, right = of pizza] (luigi) {\alert<3>{\sf Luigi}};
\node [dnode, above right = of luigi] (cash) {{\sf cash}};
\node [ndnode, right = of burger] (mc) {\sf McBurger};
\draw[darr] (burger) to node[above] {$\delta_7$} (mc);
\draw[darr] (mc) to node[above] {$\delta_8$} (cash);
