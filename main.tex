...
\pause
In our example: $\mathbb{K} \nc_{1/2} \alert{{\sf pro\mbox{-}science}}$.
...
Disjunctive Detachment:
    \begin{align*}
     \infer[\bf DisDet]{(\bigvee_{i=1}^n\alert{\phi_i})^{f}, \langle (\alert{\phi_{i}}, {\color{blue}\psi_i}) \rangle_{i=1}^n \Rightarrow (\bigvee_{i=1}^n {\color{blue}\psi_i})^b} {\vphantom{M}}
     \end{align*}
\pause 

     Disjunctive Defeat:
     \begin{align*}
     \infer[\bf DisDef]{(\bigvee_{i=1}^n \phi_i)^f, \langle (\phi_{i}, \psi_i) \rangle_{i=2}^n, \alert{\neg\psi_1}^b \Rightarrow \neg(\phi_1, \alert{\psi_1})}{\vphantom{M}}
    \end{align*}
\end{frame}
...
\scalebox{.8}{
    ...
    \irr2{Cut}{\seqq{(\burger \vee \pizza)^b, \d_2, \d_3, \d_4, \d_5}{\cash^b}}
    \irl2{Cut}{\seqq{\Lunch^f, \d_1,\ldots,\d_5}{\cash^b}}
    \DP
    \]
}
\end{argbox}    
...
\scalebox{.9}{
    ...
    \axs{}
    \irl1{DisDef}{\seqq{(\burger \vee \pizza)^b, \neg \mc^b, \d_3}{\neg \d_2}}
    \irl2{Cut}{\seqq{\Monday^f, (\burger\vee\pizza)^b, \d_3, \d_6}{\neg \d_2}}
...
\begin{argbox}
    ...
    \node [os, right = of c] (d) {\afnodeflat{d}{\Lunch^f, \Monday^f, \d_1, \d_3 \d_6}{\neg \d_2}};
...
\begin{frame}{Disjunctive Default Logic}
    In a forthcoming paper we define a version of Reiter's greedy algorithm for disjunctive reasoning. We show that every default extension corresponds to a stable set and vice versa.

    This generalizes our characterization of Reiter's default logic and demonstrates the merits of embedding Reiter's logic in a principles proof-theoretic approach.
...
\draw[purple, darr] (burger) to node[above] {\( \delta_2 \)} (mc);
\draw[purple, darr] (pizza) to node[above] {\( \delta_3 \)} (luigi);
\draw[purple, darr] (mc.0) to [bend left] node[above] {\( \delta_4 \)} (cash);
\draw[purple, darr] (luigi.0) to [bend right] node[above] {\( \delta_5 \)} (cash);
...
\begin{frame}{DAC with Hypothetical Arguments}
\begin{tikzpicture}[scale=.8, every node/.style={scale=.8, align=center}, rounded corners, decorate, node distance = .3cm and 1.8cm]
\node [] (v) {\( \vee \)};
\node [ndnode, above = of v] (burger) {{\sf burger}};
\node [ndnode, below = of v] (pizza) {{\sf pizza}};
...
\draw[darr] (wedge) to node[above] {$\delta_2$} (king);
\node [ospale,right = of king] (ncash) {{\alert<6>{$\neg$ {\sf cash}}}};
